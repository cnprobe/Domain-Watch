...
- **后缀列是可搜索的下拉框**：输入即过滤，候选来自 IANA 完整 TLD 列表（约 1400 条，缓存 72 小时）并合并 IANA 引导文件与已配置的自定义后缀
  - 匹配规则：完全匹配 → 前缀匹配 → 包含匹配，结果按相关度排序
  - **没有匹配项时会出现「将新建此映射」**，直接回车或点击即可用输入内容创建
  - 支持键盘：`↑` `↓` 选择，`Enter` 确认，`Esc` 关闭；点击输入框或右侧箭头展开
