...
四个页面共用同一套 Swiss International（瑞士国际风格）设计规则，样式内联在各自 HTML 的 `<style>` 里，没有构建步骤。改动时守住下面几条，避免风格漂移：

- **配色只有黑白红**：`#000000` / `#ffffff` / `#ff0000`，层次靠 `#cccccc` 细线、`#f0f0f0` 悬停底与两档灰字完成，不要引入其他色相
- **无衬线是唯一字族**（Helvetica 栈）：不出现衬线体、等宽体、斜体
- **零阴影、零圆角、零渐变、零背景图案、零虚线边框**
- **过渡只改颜色、边框色与不透明度**，一律 `150ms ease-out`；禁止 translate / scale / 阴影，唯一的移动是按钮箭头
- **方角 + 1px 黑边**是所有按钮、卡片、输入框的统一形态，焦点用红边表示而不是光晕
- **按钮必须带方向箭头**（CSS `::after` 的 `→`），导航链接同理
- **引线延伸**：列表行、统计块、汇总块的左侧 1px 灰线在悬停时变红，背景转 `#f0f0f0`——网格被激活而不是被装饰
- 标题粗体紧字距，标签全大写宽字距，正文左对齐，行宽不超过 68 字符
- 基础层定义在每个页面 `<style>` 的第一段，**四页必须逐字一致**；改一处就要同步四处
...
- 四个页面都带 `Cache-Control: no-cache` 加 `ETag`：浏览器和代理每次回源校验，内容没变回 `304`（省流量），改动界面后立刻生效。**代理侧不要覆盖成 `Cache-Control: public, max-age=...`**，否则私有页面可能被共享缓存留存。
- 登录跳转一律 `Cache-Control: no-store`，不会出现「登录后访问 `/settings` 仍被弹回登录页」。
- 所有 `/api/*` 响应都是 `no-store`。
- 自定义页面文件（`PUBLIC_FILE` 等）按 mtime 判定，改完文件下一次请求就生效，不必重启容器。
